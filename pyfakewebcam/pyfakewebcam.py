...
class FakeWebcam:
    def __init__(self, video_device, width, height, channels=4, input_pixfmt='BGRA'):
        ...
        if channels != 4:
            raise NotImplementedError('Code only supports inputs with 4 channels right now. You tried to intialize with {} channels'.format(channels))

        if input_pixfmt != 'BGRA':
            raise NotImplementedError('Code only supports BGRA pixfmt. You tried to intialize with {}'.format(input_pixfmt))
        ...
        self._bgra = np.zeros((self._settings.fmt.pix.height, self._settings.fmt.pix.width, 3), dtype=np.uint8)
